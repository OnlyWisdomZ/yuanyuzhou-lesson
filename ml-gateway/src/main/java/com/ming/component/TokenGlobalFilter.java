...
public class TokenGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (StringUtils.isBlank(token)) {
            return buildResponseData(response, 1004, "登录过期", "Token不存在", "");
        }
        // 解析Token令牌
        Map<String, Object> verifyResult;
        try {
            verifyResult = JwtUtil.parse(token);
        } catch (RuntimeException e) {
            return buildResponseData(response, 1004, "登录过期", "Token解析失败", "");
        }
        // 若Token令牌即将过期，则返回过期提示以及一个新的Token令牌
        if ((boolean) verifyResult.get("expiringSoon")) {
            return buildResponseData(response, 1005, "登录即将过期", "Token即将过期", verifyResult.get("newToken"));
        }
        // 放行请求
    ...
    private Mono<Void> buildResponseData(ServerHttpResponse response,
        ...
        // 设置响应数据（Map）
        Map<String, Object> resultMap = Map.of("code", code, "message", message, "coderMessage", coderMessage, "data", data);
